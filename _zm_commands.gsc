...
function __init__()
{
    ...
    thread _spawn_portal_cube_command_response();
...
function private _spawn_portal_cube_command_response(command_args)
{
    ModVar("spawn", ""); // Change ModVar to "spawn" so you can use "/spawn ..."
    ...
    for (;;)
    {
        ...
        if (isdefined(dvar_value) && dvar_value != "")
        {
            ...
            // Split args: supports `/spawn cube` or `/spawn laser_cube`
            tokenized = StrTok(dvar_value, " ");
            arg = "";
            if (tokenized.size > 0)
                arg = tokenized[0];
            ...
            // Optional: allow a player index as second argument
            if (tokenized.size > 1)
                player_index = Int(tokenized[1]);
            else
                player_index = 0; // default to first player

            // What to spawn
            spawn_func = &spawn_portal_cube;
            if (arg == "laser_cube")
                spawn_func = &spawn_laser_cube;

            // Player selection: if index valid, spawn for that player; otherwise for all
            if (player_index >= 0 && player_index < GetPlayers().size)
            {
                level.players[player_index] thread [[spawn_func]]();
                print_subtitle(level.players[player_index], "^5Dev: Spawned " + arg + " for " + level.players[player_index].playername);
            }
            else
            {
                foreach (player in GetPlayers())
                {
                    player thread [[spawn_func]]();
                }
                print_subtitle(undefined, "^5Dev: Spawned " + arg + " for all players");
            }
        }
    }
}

function spawn_portal_cube()
{
    // Find spawn position ~50 units in front of player, 40 units up (to avoid ground)
    forward = AnglesToForward(self.angles);
    spawn_pos = self.origin + (forward * 50) + (0, 0, 40);

    // Replace "model_portal_cube" with your actual model name if needed
    cube = Spawn("script_model", spawn_pos);
    cube SetModel("m_0a7141f5_metal_box_clean"); // <-- your model's name
    cube.angles = (0, self.angles[1], 0); // face same direction as player
    cube.targetname = "portal_cube";
    //cube clientfield::set( "model_change_color", 1 );
    cube Solid();

    // Optional: Physics enable, etc. – adjust if needed for your setup!
    cube PhysicsLaunch(cube.origin, (0, 0, 0));
}

function spawn_laser_cube()
{
    // Find spawn position ~50 units in front of player, 40 units up (to avoid ground)
    forward = AnglesToForward(self.angles);
    spawn_pos = self.origin + (forward * 50) + (0, 0, 40);
    ...
    // Replace "model_portal_cube" with your actual model name if needed
    cube = Spawn("script_model", spawn_pos);
    cube SetModel("m_0a7141f5_reflection_cube"); // <-- your model's name
    cube.angles = (0, self.angles[1], 0); // face same direction as player
    cube.targetname = "portal_cube";
    //cube clientfield::set( "model_change_color", 1 );
    cube Solid();
